...
* **Прикладной уровень** позволяет приложению взаимодействовать с сетью.
...
* **Уровень представления** обеспечивает преобразование данных в формат, пригодный для передачи и понятный получателю.
* Кодировки ASCII, Unicode; протоколы SSL/TLS для шифрования; сжатие MPEG, MP3, JPEG, ...

> 3. Данные спускаются на уровень представления. Запрос формматируется для передачи по сети, а данные кодируются.

* **Cеансовый уровень** - управляет сеансами связи. Показывает: какой тип соединения будет применён.
* Типы: ***симплексное*** (в одном напралении передача), ***полудуплексное*** (в обоих направлениях, но неодновременно), ***дуплексное*** (одновременно в обоих направлениях).
* Протокол SIP, ...
...
* **Траснпортный уровень** обеспечивает передачу данных между отправителем и получателем.
...
* **TCP** обеспечивает надёжную передачу.

Использует ***правило тройного рукопожатия***:
* устройство А отправляет сегмент с флагом SYN (что хочет начать передачу)
* устройство В отпраляет сегмент SYN+ACK (готов принимать передачу)
* устройство А отправляет сегмен с флагом ACK (мы оба согласны, начинаю передачу).
...
* Обеспечивает быструю передачу, но не гарантирует надёжность, порядок сегментов, целостность данных при передаче.
...
* **Сетевой уровень** обеспечивает маршрутизацию данных по сети.
...
* **Канальный уровень** управляет доступом среде передачи данных, включая адресацию (MAC).
* Т.е. обеспечивает, чтобы данные дошли по правильному MAC-адресу, помогает передавать данные по общей среде. Также занимается проверкой и исправлением данных (LLC).
...
* **Физический уровень** отвечает за физическую передачу данных через среду (кабели, радиоволны, ...).
...
> Cтоит отметить, что хабы понимают только физический уровень, коммутаторы - физический и канальный, маршрутизаторы - физический и канальный и сетевой. Когда через них проходят данные, они декапсулируются и инкапсулируются для прохождения дальше по сети, как это было конечными устройствами.

Больше подробностей можете узнать тут: https://2hpc.ru/%D0%BC%D0%BE%D0%B4%D0%B5%D0%BB%D1%8C-osi-%D1%83%D1%80%D0%BE%D0%B2%D0%BD%D0%B8-%D0%BF%D1%80%D0%BE%D1%82%D0%BE%D0%BA%D0%BE%D0%BB%D1%8B-%D1%84%D1%83%D0%BD%D0%BA%D1%86%D0%B8%D0%B8/#challenge_verified
