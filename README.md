...
На данной картинке изображены конечные и промежуточные устройства, а также линия линии связи (сетевые среды/среда передачи данных).

<img width="491" height="91" alt="image" src="https://github.com/user-attachments/assets/48ff5b6f-ed20-43af-b891-fbb202a966fa" />

На данной картинке изображены в верхнем левом углу значок молнии - среда передача данных или кабеля (более простым языком). 
...
На данной картинке изображены в верхнем нижним углу находятся конечные устройства.
...
На данных картинках изображены: 1 - хабы, 2 - коммутаторы, 3 - маршрутизаторы (по-английски роутер).

* "Промежуточные устройства" - хаб, коммутатор, маршрутизатор, и т.д. Управляют процессом передачи данных (направляют, фильтруют данные), обеспечивают связь между сетями. Также обмениваются служебной информацией.

** "Повторители" и "хабы" используются, когда устройства находятся на большом расстоянии. Причина в том, что в физической среде происходит постепенное затухание сигнала. Данные устройства распознают сигнал и повторяют в первоначальном виде. Обычно у витой пары/прямой и кроссовер 100 метров дальность сигнала. Если захотим что-то передать, что первышает это расстояние, понадобится более качественные кабеля или промежуточные устройства.
...
<img width="458" height="327" alt="image" src="https://github.com/user-attachments/assets/a5fc08a4-9e3c-4705-b49b-7c876f6f3986" />
  ...
  > Повторители и хабы работают на физическом уровне модели OSI.
...
* На уровне представление, прикладном и сеансовом происхожит работа с данными. Здесь работают такие протоколы, как HTTP, DNS, DHCP, ...
  
* На транспортном уровне работа с сегментами/датаграммами. Задача: обеспечить передачу данных между устройствами. Здесь данные разбиваются на сегменты (так как размер данных может быть очень большиим, их проще передавать по отдельности, маленькими кусочками). Сегмент состоит из заголовка и поля данных. На этом уровне добавляется дополнительная информация: логический порт получателя и источника. Часто используемые протоколы этого уровня TCP, UDP.
  
* На сетевом урвоне работа с пакетами. Сегменты разбивются на пакеты.  
* На канальном уровне с кадрами.
* На физическом уровне 0 и 1.
...
2. На уровне представления обычно данные форматируется под другое устройство, чтобы могло с пришедним данными работать. Здесь происходит кодирование/декодирование, шифрование/дешифрование,сжатие данные
