...
+ ***Повторители*** и ***хабы*** используются, когда устройства находятся на большом расстоянии.
...
> У хабов есть одна проблема. Если на 2 порта и более прийдёт одновременно сигнал, то произойдёт коллизия (информация столкнётся, перемешается и восстановить нельзя будет). В данный момент их редко, где используют, на смену пришли коммутаторы.

* ***Повторители*** и ***хабы*** работают на физическом уровне модели OSI (о нём чуть позже). Этот уровень использует ***биты***.
...
+ Основная задача ***коммутатора*** и ***сетевого моста*** обеспечивать связь ***внутри локальной сети*** (передача данных от 1 устройства к другому, в рамках одного адрес сети или подсети (об этом чуть позже)).
...
* ***Маршрутизатор*** обеспечивают между локальными сетями, передаёт данные между ними. Его задача определить путь, по которому будут проходить пакеты. Для этого используется статическая или динамическая маршрутизация (о ней чуть позже).
...
1. Персональные сети - передача данных между устройствами, которые находятся рядом друг с другом. Пример: часы и смартфон. Расстояние 10-20см до пары метров. Могут взаимодейтсвовать с другими сетями. Используют Wi-Fi, Bletooth,...
   
2. Локальная сеть - объединяет устрйства на небольшом расстоянии. Не зависит от других сетей. Пример: офис.
   
3. Виртуальная приватная сеть - для безопасного и защищённого соединения. Подключения отдельных серверов с обеих сторон. Пример: у работодателя есть такая сеть для доступа к закрытым ресурсам для определённых сотрудников.
   
4. Глобальная сеть - объединяет между собой неогрраниченное количество локальных, региональных сетей, может находиться в нескольких странах.

> Глобальная зональная сеть - больше, чем обычная глобальная. Пример: интернет.

#### Топология

Теперь про топологию. Топология бывает физическая и логическая. В практических работах используем логическую. Виды:

* ***полносвязная***. Все устройтсва соединены между собой. Из +: надёжность. Из -: закупка дополнительных кабелей.
  
* ***шинная***. Все устройства подключены к 1 кабелю, передача данных по шине. Из +: дёшево. Из -: поломки из-за высконой нагрузки, также ограниченная длина кабеля.
  
* ***звезда***. Для связи между устройствами используется центральный узел. В данном случае промежуточные устройства. Из +: хорошая изоляция устройств и простота в расширении (подключил к порту). Из -: если сломался центральный узел, то сеть упала, если сломался пк, то сеть дальше функционирует (это +).
  
* ***кольцевая***. Передача данных по кольцу. Из +: надёжность. Из -: если сломался пк, то прийдётся его убрать, чтобы сеть дальше функционировала.
...
> Раньше использовалось деление на классы, но вскоре их отменили из-за большое количества пользователей в интернете, на смену пришла CIDR. Частные сети используются.
...
Мы разобрались в базовой терминологии компьютерных сетей. Теперь перейдёи к практическим работам. Где разберём ещё пару тем, терминов (модель TCP/IP и OSI, DNS, DHCP, TCP, UDP) для полного усовения базовой теории по компьютерным сетям.
