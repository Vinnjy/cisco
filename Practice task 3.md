...
> Причина заключается в том, что у пк указан шлюз, с помощью него передача может выходить за локальную сеть. Чтобы выйти, нужно прийти на соответствующий сетевой интерфейс (в этом помогает таблица маршрутизации).
...
> Так как их соединяет хаб, нам упрощает задачу. Поэтому на всех сетевых интерфейсах fa0/0 всех маршрутизаторов укажем IP-адреса из общей сети, понятная провайдеру.

> В итоге, провайдер станет переправной точкой для трафика (посмотрите в симуляции,что происходит с DNS).
