...
> Причина заключается в том, что у пк указан шлюз, с помощью него передача может выходить за локальную сеть. Шлюз выйдет на один сетевой интерфейс маршрутизатора, которыый используется для подключения к внутренней локальной сети, другой — для связи с сетью провайдера (в этом помогает таблица маршрутизации). 
